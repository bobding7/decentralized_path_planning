...
module.  These are:
    ...
    1.  Grid_Graph and Grid_Graph_Conn_8: These subclass Graph_Interface
        ...
        Added: Grid_Graph_Turn: specifies a configuration space with 4 
        connectivity and turning; i.e., each robot can only turn 90 degrees
        or, if it is facing the correct direction, go to the space immediately 
        above its current position, below its current position, or to the left 
        or right of its current position.
...
class Grid_Graph(interface.Graph_Interface):
    ...
    def get_neighbors(self, coord):
        ...
        coord - (x, y, h) coordinate of the node for which neighbors are
                being generated

        Return value in form of list of (x, y, h) tuples giving coordinates
...
class Grid_Graph_Turn(interface.Graph_Interface):
    """ Represents configuration space for grid workspace
    ...
    that a robot can go to the grid spaces located one space
    up, one space to the left, one space to the right, and one
    space down from its current coordinate. The robot must have
    a heading in the correct direction to move in that direction,
    otherwise it must first turn (change it's heading)
    """
    ...
    def get_neighbors(self, coord):
        ...
        # turn
        neighbors.append((coord[0], coord[1], (coord[2] % 4) + 1))
        neighbors.append((coord[0], coord[1], ((coord[2] - 2) % 4) + 1))

        # move in direction
        if (coord[2] == Heading.NORTH):
            new_coord = (coord[0], coord[1] - 1, Heading.NORTH)
        elif (coord[2] == Heading.EAST):
            new_coord = (coord[0] + 1, coord[1], Heading.EAST)
        elif (coord[2] == Heading.SOUTH):
            new_coord = (coord[0], coord[1] + 1, Heading.SOUTH)
        elif (coord[2] == Heading.WEST):
            new_coord = (coord[0] - 1, coord[1], Heading.WEST)
...
class GridGraphTurn4WaitAtGoal(Grid_Graph_Turn):
    """Variant of workspace_graph.Grid_Graph_Turn that allows for the robot
    ...
    def __init__(self, world_descriptor, goal, wait_cost=.0,
        ...
        """
        super(GridGraphTurn4WaitAtGoal, self).__init__(
    ...
    def get_edge_cost(self, coord1, coord2):
        ...
        return super(GridGraphTurn4WaitAtGoal, self).get_edge_cost(coord1,
                                                                   coord2)
...
def compute_heuristic_conn_4(init_pos, coord):
    ...
    """
    if init_pos[2] == 4 and coord[2] == 1:
        turn_dis = 1
    elif init_pos[2] == 1 and coord[2] == 4:
        turn_dis = 1
    else:
        turn_dis = abs(init_pos[2] - coord[2])
    return (abs(init_pos[0] - coord[0]) +
            abs(init_pos[1] - coord[1]) +
            turn_dis)
...
def Astar_Graph(world_descriptor, goal=None, connect_8=False,
    ...
    return Astar_Policy(world_descriptor,
                        lambda x: GridGraphTurn4WaitAtGoal(
                            x, goal, wait_cost=.0,
                            diagonal_cost=diagonal_cost),
                        goal, compute_heuristic_conn_4)


def Astar_Turn_Graph(world_descriptor, goal=None, connect_8=False,
                     diagonal_cost=False, makespan=False):
    """Wrapper function for returning Astar_Policy objects
...
class Edge_Checker(interface.Planner_Edge_Checker):
    ...
    def col_check(self, c1, recursive):
        ...
        for i in xrange(len(c1) - 1):
            for j in xrange(i + 1, len(c1)):
                # heading doesn't matter for collisions
                if c1[i][0:2] == c1[j][0:2]:
