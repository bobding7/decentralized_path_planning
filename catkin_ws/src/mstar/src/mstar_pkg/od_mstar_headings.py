...
import workspace_graph_headings
import sys
...
MAX_COST = workspace_graph_headings.MAX_COST
...
def find_path(obs_map, init_pos, goals, recursive=True, inflation=1.0,
    ...
    obs_map           - obstacle map,  matrix with 0 for free,  1 for
                        obstacle
    init_pos          - ((x1, y1, h), (x2, y2, h), ...) coordinates of the
                        initial state, should be tuples of xy position and
                        heading ("N","S","E","W")
    goals             - ((x1, y1, h), (x2, y2, h), ...) coordinates of the goal
                        should be tuples
    ...
    if (col_checker is None or isinstance(col_checker,
                                          workspace_graph_headings.Edge_Checker)):
...
class Od_Mstar(object):
    ...
    def __init__(self, obs_map, goals, recursive, sub_search=None,
        ...
        obs_map           - obstacle map,  matrix with 0 for free,  1
                            for obstacle
        goals             - ((x1, y1, h), (x2, y2, h), ...) coordinates 
                            of the goal, should be tuples
        recursive         - True for rM*, false for basic M*
        sub_search        - Sub planners, should be None for the full
                            configuration space
        col_checker       - object to handle robot-robot collision checking.
                            Should implement the same interface as 
                            workspace_graph_headings.Edge_Checker
        ...
        if self.col_checker is None:
            self.col_checker = workspace_graph_headings.Edge_Checker()
        ...
        if self.sub_search is None:
            ...
            if self.flood_fill_policy:
                for dex, key in enumerate(self.policy_keys):
                    self.sub_search[key] = workspace_graph_headings.Workspace_Graph(
                        obs_map, goals[dex], connect_8=self.connect_8)
            else:
                for dex, key in enumerate(self.policy_keys):
                    self.sub_search[key] = workspace_graph_headings.Astar_DiGraph_Policy(
                        obs_map,
                        lambda x: workspace_graph_headings.GridGraphTurn4WaitAtGoal(
                            x, goals[dex], wait_cost=.0),
                        goals[dex], workspace_graph_headings.compute_heuristic_conn_4)
    ...
    def find_path(self, init_pos, time_limit=5 * 60):
        ...
        init_pos   - ((x1, y1, h), (x2, y2, h), ...) coordinates of 
                     initial position
        ...
        if time_limit > 0:
            ...
            for planner in self.sub_search.itervalues():
    ...
    def gen_epeastar_coords(self, node):
        ...
        for rob_dex in xrange(self.num_bots):
    ...
    def epeastar_transition_cost(self, start_coord, prev_cost, new_coord):
        ...
        """
        for dex, (source, target) in enumerate(izip(start_coord, new_coord)):
    ...
    def get_neighbors_recursive(self, node):
        ...
        new_coord = [self.sub_search[self.policy_keys[i]].get_step(
            start_coord[i]) for i in xrange(self.num_bots)]
...
class mstar_node(object):
    ...
    coordinates of the form ((x1, y1, h1), (x2, y2, h2), ...),  while
    intermediate nodes have coordinates of the form (((x1, y1, h1), ...),
    move_tuple)
    """
...
def find_path_limited_graph(obs_map, init_pos, goals, recursive=True,
    ...
    for i in xrange(len(o.goals)):
        o.sub_search[(i, )] = workspace_graph_headings.Networkx_Graph(
