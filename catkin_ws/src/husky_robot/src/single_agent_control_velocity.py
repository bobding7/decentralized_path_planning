...
class singleagent_path_tracking():
    ...
    # Find paths given the goal position
        ...
        self.path = pickle.load(open("/home/foxconn/foxconnmultiagent/catkin_ws/src/husky_robot/src/data/path_12agents.p"))
        ...
        for i in range(0, len(self.path)-1):
            if not (self.path[i][self.agent_num - 1][0]==self.path[i+1][self.agent_num - 1][0] and self.path[i][self.agent_num - 1][1][2]==self.path[i+1][self.agent_num - 1][1][2]):
                ...
                if not (euclidean_dist < 0.01):
                    # print("translation")
                    numSteps = int(math.floor(euclidean_dist/forwardSpeed*frequency))
                    agent_state = getModelStates.gms_client('Husky_' + 'agent' + str(self.agent_num),'world')
                    agent_pose = [agent_state.pose.position.x, agent_state.pose.position.y, 
                                    math.copysign(2*math.acos(agent_state.pose.orientation.w),agent_state.pose.orientation.z)]
                    startPosition = copy(agent_pose)
                    # for x in range(0, numSteps):
                    distance_travelled = 0
                    while (distance_travelled<1):
                        # print(self.agent_num, i, euclidean_dist, distance_travelled, startPosition[0], agent_pose[0], startPosition[1], agent_pose[1])
                        agent_state = getModelStates.gms_client('Husky_' + 'agent' + str(self.agent_num),'world')
                        agent_pose = [agent_state.pose.position.x, agent_state.pose.position.y,  
                                        math.copysign(2*math.acos(agent_state.pose.orientation.w),agent_state.pose.orientation.z)]
                        currentPosition = copy(agent_pose)
                        distance_travelled = math.sqrt((currentPosition[0] - startPosition[0])**2 + (currentPosition[1] - startPosition[1])**2)/euclidean_dist 
                        if(distance_travelled<0.8):
                            move_cmd_x.linear.x = forwardSpeed
                        else:
