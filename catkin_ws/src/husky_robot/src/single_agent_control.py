...
class singleagent_path_tracking():
    ...
    def single_agent_motion(self):
        ...
        angular_speed = radians(10) #angular_speed deg/s converted into radians/s
        turn_cmd = Twist()
        turn_cmd.linear.x = 0
        ...
        for i in range(0, len(self.path)-1):
            if not (self.path[i][self.agent_num - 1][0]==self.path[i+1][self.agent_num - 1][0] and self.path[i][self.agent_num - 1][1][2]==self.path[i+1][self.agent_num - 1][1][2]):
                ...
                while (math.fabs(delta_angle) > 0.01):
                    ...
                    turn_cmd.angular.z = math.copysign(angular_speed, delta_angle)
                    if (math.fabs(delta_angle) < 0.2):
                        turn_cmd.angular.z = math.copysign(0.1, delta_angle)
                ...
                leftOverTime = startTime + loopTime - rospy.get_time()
                if leftOverTime < 0:
                    print("agent: %d has negative Time left after rotation" %self.agent_num)
                elif (euclidean_dist > 0.01):
                    # print("translation")
                    for x in range(0, int(math.floor(leftOverTime*frequency))):
                        # print(self.agent_num, i, euclidean_dist, angle, agent_pose[2], xInitial, xFinal, yInitial, yFinal)
                        move_cmd_x.linear.x = euclidean_dist/leftOverTime
                ...
                leftOverTime = startTime + loopTime - rospy.get_time()
                if leftOverTime < 0:
                    print("agent: %d has negative Time left after translation" %self.agent_num)
                else:
                    rospy.sleep(leftOverTime)
                
                startTime = startTime + loopTime

            else:
                rospy.sleep(loopTime)
                startTime = startTime + loopTime
